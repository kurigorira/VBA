...
Private Function IsTodayDischarge(dischStr As String) As Boolean
    ...
    ' 本日と比較（yyyy/m/d と yyyy/mm/dd 両形式に対応）
    Dim todayStr As String
    Dim todayStrZ As String
    todayStr  = Format(Now, "yyyy/m/d")    ' 例: 2026/6/2
    todayStrZ = Format(Now, "yyyy/mm/dd")  ' 例: 2026/06/02
...
Private Function IsInPeriod3(periodStr As String) As Boolean
    ' ChrW(9314)=③ ChrW(9312)=① ChrW(9313)=②  ※Chr()は255超え非対応のためChrW()使用
    If InStr(periodStr, ChrW(9314)) > 0 Then
        IsInPeriod3 = True : Exit Function
    End If
    ' ③文字が取れなくても入院期間があり①②超えでなければ③と判定
    If InStr(periodStr, "入院期間") > 0 Then
        If InStr(periodStr, ChrW(9312)) = 0 And _
           InStr(periodStr, ChrW(9313)) = 0 And _
           InStr(periodStr, "超え") = 0 Then
            IsInPeriod3 = True : Exit Function
        End If
    End If
    IsInPeriod3 = False
...
Private Function GetRowType(p As PatientData) As String
    ...
    ElseIf InStr(p.period, ChrW(9313)) > 0 Or _
           (InStr(p.period, "入院期間") > 0 And InStr(p.period, "②") > 0) Then
        GetRowType = "PERIOD2"
